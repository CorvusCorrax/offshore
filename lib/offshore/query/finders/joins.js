...
var Joins = module.exports = function(joins, values, identity, schema, collections) {
  ...
  _.each(joins || [], function(join) {
    // If the value is not being selected, don't add it to the index
    if (!join.select) return;

    if (join.path) {
      var pathChunks = join.path.split('.');
      var parent = pathChunks.shift();
      var child = _.clone(join);
      child.path = pathChunks.join('.');
      self.joinsIndex[parent].related = self.joinsIndex[parent].related || [];
      self.joinsIndex[parent].related.push(child);
      return;
    }
    self.joinsIndex[join.alias] = _.clone(join);
  });
...
Joins.prototype.modelOptions = function modelOptions() {
  ...
  this.options = {
    showJoins: !!_.keys(this.joinsIndex).length
  };

  // If no joins were used, just return
  if (!this.options.showJoins) return;
  ...
  this.options.joins = _.keys(this.joinsIndex).map(function(key) {
    var join = self.joinsIndex[key];
    var child = [];
...
Joins.prototype.modelize = function modelize(value) {
  ...
  Object.keys(value).forEach(function(key) {
    ...
    var options = { showJoins: false };
    // check if deep joins were related
    if (usedInJoin.join.related && usedInJoin.join.related.length) {
      options = {
        showJoins: true,
        joins: usedInJoin.join.related
      };
      var identity = joinKey || usedInJoin.join.child;
      var schema = self.collections[identity].offshore.schema[identity];
      new Joins(usedInJoin.join.related, value[key], identity, schema, self.collections);
    }
    ...
    if (Array.isArray(value[key])) {
      ...
      value[key].forEach(function(val) {
        ...
        if (joinKey) {
          ...
          model = new collection._model(val, options);
    ...
    if (!_.isNull(value[key])) {
      if (_.isObject(value[key])) {
        ...
        var collection = self.collections[joinKey || usedInJoin.join.child];
        value[key] = collection._transformer.unserialize(value[key]);
        value[key] = new collection._model(value[key], options);
      } else {
        value[key] = null;
      }
    }
...
Joins.prototype.checkForJoin = function checkForJoin(key) {
  ...
  if (this.joinsIndex[key]) {
    usedInJoin = true;
    relatedJoin = this.joinsIndex[key];
  }
