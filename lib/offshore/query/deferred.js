...
Deferred.prototype.whereDeep = function(ParentName, where, cb) {
  ...
  async.map(_.keys(values), function(property, next) {
    ...
    if (hasOwnProperty(attributes[property], 'collection')) {
      ...
      collections[childName]._loadQuery(self._query).find(childCollectionCriteria).exec(function(err, data) {
        if (err) {
          return next(err);
        }
        var via = collections[ParentName].attributes[property].via;
        ...
        collections[junctionTable]._loadQuery(self._query).find(junctionCriteria, function(err, data) {
          if (err) {
            return next(err);
          }
          next(null, {
      ...
      // resolve belongsTo
      var child = collections[childName]._loadQuery(self._query);
