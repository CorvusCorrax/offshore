...
var Deferred = module.exports = function(context, method, criteria, values) {
  ...
  this._criteria = normalize.criteria(criteria);

  this._criteria.paths = this._criteria.paths || {};
  var pk = _.find(_.keys(this._context.attributes), function(attr) {
    return self._context.attributes[attr].primaryKey;
  });

  this._criteria.paths[context.identity] = this._criteria.paths[context.identity] || {primaryKey: pk, associations: {models: {}, collections: {}}};
...
Deferred.prototype.toString = function() {
  ...
  if (this._criteria.paths) {
    ...
    paths.forEach(function(path) {
      var joins = self._criteria.paths[path].joins;
      // sort joins by alias
...
Deferred.prototype.populate = function(keyName, criteria) {
  ...
  try {
    ...
    join = {
      ...
      path: this._context.identity + '.' + keyName
    };

    var schema = this._context.offshore.schema[attr.references];
    var reference = null;

    this._criteria.paths[join.path] = this._criteria.paths[join.path] || {associations: {models: {}, collections: {}}};
    if (join.collection) {
      this._criteria.paths[this._context.identity].associations.collections[keyName] = {
        collectionName: parentKey.collection,
        primaryKey: _.find(_.keys(this._context.offshore.collections[parentKey.collection].attributes), function(attr) {
          return self._context.offshore.collections[parentKey.collection].attributes[attr].primaryKey;
        }),
        via: parentKey.via
      };
    } else {
      this._criteria.paths[this._context.identity].associations.models[keyName] = {
        collectionName: parentKey.model,
        primaryKey: _.find(_.keys(this._context.offshore.collections[parentKey.model].attributes), function(attr) {
          return self._context.offshore.collections[parentKey.model].attributes[attr].primaryKey;
        })
      };
    }
    ...
    _.each(this._context.offshore.schema[attr.references].attributes, function(val, key) {
      ...
      if (!_.has(val, 'columnName') && select.indexOf(key) === -1) {
        select.push(key);
        return;
      }

      if (select.indexOf(val.columnName) === -1) {
    ...
    if (select.indexOf(childPk) === -1) {
      select.push(childPk);
    }

    // Add the foreign key for collections
    if (join.collection && select.indexOf(attr.on) === -1) {
      select.push(attr.on);
    }
    ...
    if (reference && hasOwnProperty(attr, 'on')) {
      // Update first join as junction table
      join.path = this._context.identity + '.' + attr.references;
      this._criteria.paths[this._context.identity].associations.collections[attr.references] = {
        collectionName: attr.references,
        primaryKey: reference.columnName,
        via: attr.on,
        junctionTable: true
      };

      this._criteria.paths[this._context.identity + '.' + attr.references] = {associations: {models: {}, collections: {}}};
      this._criteria.paths[this._context.identity + '.' + attr.references].associations.models[join.alias] = {
        collectionName: reference.references,
        primaryKey: reference.onKey,
        via: reference.columnName
      };
...
Deferred.prototype.populatePath = function(path, criteria) {
  ...
  for (var j = 0; j < pathChunks.length; j++) {
    ...
    if (!this._criteria.paths[currentPath]) {
      this._criteria.paths[currentPath] = {associations: {models: {}, collections: {}}};
    }

    var associations = {};
    if (parentAttributes[currentAlias].collection) {
      associations = this._criteria.paths[currentPath].associations.collections;
    } else if (parentAttributes[currentAlias].model) {
      associations = this._criteria.paths[currentPath].associations.models;
    }

    // if true, the alias does not exist in the current path, adding it
    if (!associations[currentAlias]) {
      var parent = this;
      ...
      if (_.last(pathChunks) === currentAlias) {
        populate = parent.populate(currentAlias, criteria);
      } else {
        populate = parent.populate(currentAlias);
      }
      var joins = populate._criteria.joins;

      _.keys(populate._criteria.paths).forEach(function(path) {
        var rebasedPath = [currentPath].concat(path.split('.').slice(1)).join('.');
        self._criteria.paths[rebasedPath] = _.merge(self._criteria.paths[rebasedPath], populate._criteria.paths[path]);
      });

      joins.forEach(function(join) {
        join.path = [currentPath].concat(join.path.split('.').slice(1)).join('.');
      });
...
Deferred.prototype.exec = function(cb) {
  ...
  // remove paths if no joins
  if (!this._criteria.joins) {
    delete this._criteria.paths;
  }
