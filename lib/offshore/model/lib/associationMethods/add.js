...
var Add = module.exports = function(collection, proto, records, cb) {
  ...
  var attributes = collection.offshore.schema[collection.identity].attributes;
...
Add.prototype.createAssociations = function(key, records, cb) {
  ...
  if (self.collection._query) {
    associatedCollection = associatedCollection._loadQuery(self.collection._query);
  }
...
Add.prototype.createNewRecord = function(collection, attribute, values, key, cb) {
  ...
  var schema = this.collection.offshore.schema[attribute.collection.toLowerCase()];
  ...
  collection.create(values, function(err, record) {
    ...
    var joinCollection = self.collection.offshore.collections[attribute.collection.toLowerCase()];
    if (collection._query) {
      joinCollection = joinCollection._loadQuery(collection._query);
    }
...
Add.prototype.updateRecord = function(collection, attribute, pk, key, cb) {
  ...
  if (junctionTable) {
    var joinCollection = this.collection.offshore.collections[attribute.collection.toLowerCase()];
    if (collection._query) {
      joinCollection = joinCollection._loadQuery(collection._query);
    }
  ...
  var attributes = this.collection.offshore.schema[collection.identity].attributes;
...
Add.prototype.createManyToMany = function(collection, attribute, pk, key, cb) {
  ...
  // Grab the associated collection's primaryKey
  var collectionAttributes = this.collection.offshore.schema[attribute.collection.toLowerCase()];
  var associationKey = collectionAttributes.attributes[attribute.onKey || attribute.on].via;

  // If this is a throughTable, look into the meta data cache for what key to use
  if (collectionAttributes.throughTable) {
    var throughAttribute = collection.attributes[attribute.onKey || attribute.on];
    associationKey = collectionAttributes.throughTable[throughAttribute.model + '.' + key];

    if (!associationKey) {
      return cb(new Error('Unable to find the proper association key in the through table definition'));
    }
  }
  ...
  async.auto({
    ...
    validateAssociation: function(next) {
      ...
      var associatedCollection = self.collection.offshore.collections[associatedCollectionName];
      if (collection._query) {
        associatedCollection = associatedCollection._loadQuery(collection._query);
      }
